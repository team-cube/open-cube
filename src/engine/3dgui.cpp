...
struct gui : g3d_gui
{
    ...
    int playerpreview(int model, int team, int weap, float sizescale, bool overlaid)
    {
        ...
        if(visible())
        {
            ...
            if(overlaid)
            {
                ...
                if(!overlaytex) overlaytex = textureload("media/interface/guioverlay.png", 3);
                gle::colorf(1, 1, 1);
    ...
    {		
        if(visible())
        {
            if(!slidertex) slidertex = textureload("media/interface/guislider.png", 3);
        ...
        if(visible())
        {
            ...
            if(icon)
            {
                if(icon[0] != ' ')
                {
                    ...
                    defformatstring(tname, "media/interface/icon/%s%s", icon, ext ? "" : ".png");
